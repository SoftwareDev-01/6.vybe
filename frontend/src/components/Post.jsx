...
function Post({ post }) {
  ...
  return (
    <div className='w-[90%] flex flex-col gap-[10px] bg-[#020217] items-center shadow-2xl shadow-[#00000058] rounded-2xl pb-[20px] text-white transition-all duration-300'>
      <div className='w-full h-[80px] flex justify-between items-center px-[10px]'>
        <div className='flex justify-center items-center md:gap-[20px] gap-[10px] cursor-pointer' onClick={() => navigate(`/profile/${post.author?.userName}`)}>
          <div className='w-[40px] h-[40px] md:w-[60px] md:h-[60px] border-2 border-white rounded-full overflow-hidden'>
            <img src={post.author?.profileImage || dp} alt="" className='w-full object-cover' />
          </div>
          <div className='w-[150px] font-semibold truncate text-white'>{post.author.userName}</div>
        </div>
        {userData._id != post.author._id &&
          <FollowButton
            tailwind={'px-[10px] minw-[60px] md:min-w-[100px] py-[5px] h-[30px] md:h-[40px] bg-white text-black rounded-2xl text-[14px] md:text-[16px] hover:scale-105 transition'}
        ...
        <div className='flex justify-center items-center gap-[10px]'>
          <div className='flex justify-center items-center gap-[5px]'>
            {!post.likes.includes(userData._id) &&
              <GoHeart className='w-[25px] h-[25px] cursor-pointer hover:scale-110 transition' onClick={handleLike} />}
            {post.likes.includes(userData._id) &&
              <GoHeartFill className='w-[25px] h-[25px] text-red-600 cursor-pointer hover:scale-110 transition' onClick={handleLike} />}
            <span>{post.likes.length}</span>
          </div>
          <div className='flex justify-center items-center gap-[5px] cursor-pointer hover:scale-110 transition' onClick={() => setShowComment(prev => !prev)}>
            <MdOutlineComment className='w-[25px] h-[25px]' />
            <span>{post.comments.length}</span>
          </div>
        </div>
        <div onClick={handleSaved} className='cursor-pointer hover:scale-110 transition'>
      ...
      {post.caption &&
        <div className='w-full px-[20px] gap-[10px] flex justify-start items-center text-gray-200'>
      ...
      {showComment &&
        <div className='w-full flex flex-col gap-[30px] pb-[20px]'>
          <div className='w-full h-[80px] flex items-center justify-between px-[20px] relative'>
            ...
            <input
              type="text"
              className='px-[10px] border-b-2 border-b-gray-400 w-[90%] outline-none h-[40px] bg-transparent text-white placeholder-gray-400'
          ...
          <div className='w-full max-h-[300px] overflow-auto'>
            {post.comments?.map((com, index) => (
              <div key={index} className='w-full px-[20px] py-[20px] flex items-center gap-[20px] border-b border-b-gray-600'>
                <div className='w-[40px] h-[40px] md:w-[60px] md:h-[60px] border-2 border-white rounded-full overflow-hidden'>
                  <img src={com.author.profileImage || dp} alt="" className='w-full object-cover' />
                </div>
                <div className='text-gray-300'>{com.message}</div>
              </div>
            ))}
          </div>
        </div>
